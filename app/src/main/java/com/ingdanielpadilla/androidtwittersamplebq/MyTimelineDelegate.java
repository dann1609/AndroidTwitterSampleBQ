...
public class MyTimelineDelegate <T extends Identifiable> {
    // once capacity is exceeded, additional items will not be loaded
    static final long CAPACITY = 2000L;
    ...
    final MyTimelineStateHolder timelineStateHolder;
    List<T> itemList;
     ...
     */
    public MyTimelineDelegate(Timeline<T> timeline) {
        this(timeline, null, null);
    }

    MyTimelineDelegate(Timeline<T> timeline, DataSetObservable observable, List<T> items) {
        if (timeline == null) {
            throw new IllegalArgumentException("Timeline must not be null");
        }
        this.timeline = timeline;
        this.timelineStateHolder = new MyTimelineStateHolder();
        if (observable == null) {
    ...
    public void refresh(Callback<TimelineResult<T>> developerCb) {
        ...
        loadNext(timelineStateHolder.positionForNext(),
                new MyTimelineDelegate.RefreshCallback(developerCb, timelineStateHolder));
    }
    ...
    public void next(Callback<TimelineResult<T>> developerCb) {
        loadNext(timelineStateHolder.positionForNext(),
                new MyTimelineDelegate.NextCallback(developerCb, timelineStateHolder));
    }
    ...
    public void previous() {
        loadPrevious(timelineStateHolder.positionForPrevious(),
                new MyTimelineDelegate.PreviousCallback(timelineStateHolder));
    }
    ...
    /**
     * MyTimelineDelegate.DefaultCallback is a Callback which handles setting requestInFlight to
    ...
    class DefaultCallback extends Callback<TimelineResult<T>> {
        ...
        DefaultCallback(Callback<TimelineResult<T>> developerCb,
                        MyTimelineStateHolder timelineStateHolder) {
    ...
    class PreviousCallback extends DefaultCallback {
        ...
        PreviousCallback(MyTimelineStateHolder timelineStateHolder) {
